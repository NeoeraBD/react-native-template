...
## 📖 4. Spec-Driven Development (OpenSpec)

Use the OpenSpec workflow for all non-trivial features and architectural changes:
1. Inspect domain specs in `openspec/specs/`.
2. Propose changes in `openspec/changes/<feature>/proposal.md` using `openspec/changes/template.md`.
3. Implement in accordance with the spec delta.
4. Verify with `yarn tsc -p tsconfig.json --noEmit && yarn lint`.
5. Archive with `yarn opsx archive <feature>`.

---

## 🛠️ 5. Compiler & Static Checks Verification
...
*   **On Pull Request / Merge**: A GitHub Actions workflow (`.github/workflows/ci.yml`) validates the entire branch before permitting a merge.
